...
- **Homepage panel art**: each homepage case-study panel is dressed with a dot-grid texture, hand-drawn UI-mockup sketches, small scribbles, two "personality" doodles, and (on three panels) a dimmed employer logo, all stacked as `background-image` layers in `src/styles/homepage-panels.css`, keyed off the `data-texture` and `data-wireframe="a|b|c|d"` attributes `CaseStudyCard` puts on each panel. Which art set (`a`–`d`, including which logo) a panel gets is set per case study by the `wireframe` field in `src/data/case-studies.ts`, not by position, so reordering the lineup keeps each logo with its own case study. The art itself is 39 SVG files in `src/img/panels/`, referenced from CSS custom properties (`--wf-a0`, `--wf-new-doodle`, `--logo-ibm`, …); at build time Vite inlines the small ones (under 4KB) and emits the rest as hashed, cacheable files. The stylesheet is imported by `CaseStudies.astro`, so only the homepage loads it. The texture never sits behind running text — `CaseStudyCard`'s `.cs-text` column has its own solid `--paper` background. Under `prefers-reduced-motion` the panels keep only the dot grid. Until September 2026 the SVGs were inlined as data URIs and the stylesheet was imported site-wide through `global.css`, which put 880KB of homepage art into every page's CSS.
