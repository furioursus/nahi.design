...
7. **Page weight — partly fixed, one decision left.** Every page was downloading about 30MB across roughly 900 requests. Two causes, both fixed on the `perf/page-weight` branch: astro-font's `preload: true` wrote 1,345 `<link rel="preload">` tags for Japanese-font subset files, so browsers pulled hundreds of them (397 files, 11.9MB, for the CV page alone); and `homepage-panels.css` carried 880KB of inlined SVG art into every page. After the fix: the homepage went from 31MB / 900 requests to 2.7MB / 49, first paint from 864ms to 132ms, and main-thread blocking from 662ms to 2ms; the IBM case study went from 30.7MB to 2.1MB. What's left: astro-font still inlines all 1,345 `@font-face` rules into every page's HTML (1.2MB raw, ~330KB gzipped, re-downloaded on every page since HTML isn't cached like a stylesheet). Removing it means self-hosting only the Latin subsets of the three Japanese typefaces — a font-loading change that needs a check that no character on the site (arrows like → and ↗, for example) falls outside those subsets.
