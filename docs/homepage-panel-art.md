...
- **Clipped to the panel** — `.panel-art` has `overflow: hidden`. The slots are fixed pixel offsets sized for desktop (one doodle sits at `left: 420px`), so on a phone several pieces land past the panel's right edge. As background layers that was harmless; as positioned elements they widened the page to 632px, and iOS Safari zoomed the whole page out to fit, leaving a blank strip down the right side. Safari also re-zooms mid-scroll as the address bar moves, which changes the page width and made the scroll script rebuild every pin — panels visibly jumped back (see [`scroll-flow.md`](scroll-flow.md) "When it falls back to plain scrolling"). Don't remove the clip.
