...
- **Pin-spacer backing.** Each `.pin-spacer` gets an opaque backing — `--shadow-tint` pre-mixed onto `--paper`, so it looks the same whatever the page background is (see "Dark closing panel") — so where gaps in the tears do line up across layers, what shows through reads as a shadowed recess rather than a clean view down the stack. On the `ink` surface that tint would come out lighter than the panel, so those spacers get a darker mix — a shadow is always darker than what it's cut into. The seam shadow is a `box-shadow`, not `drop-shadow`: `drop-shadow` reads the composited alpha of everything inside, so once the spacer has a background of its own its shadow shape would go back to a plain rectangle.
...
**TL;DR** — On a page whose last panel is on the `ink` surface (today only the homepage), the page background itself is ink, so the strip under Safari's bottom toolbar matches the dark "Get in touch" panel on phones. Pure CSS, static, no scroll logic.

- **What changes.** `html` and `body` get `background-color: var(--ink)` via `html:has(.panel[data-surface="ink"]:last-child)` in `scroll-sections.css`, applied from first paint. Every panel has its own opaque background, so the page still reads cream; the ink only shows under Safari's toolbars and in overscroll bounce.
- **Pin-spacer backing is opaque.** The torn edges show each pin-spacer's backing through their gaps. That backing used to be a translucent tint over a cream page; over an ink page it showed dark gaps. It's now the same tint pre-mixed onto `--paper`, which looks identical on every page whatever the page background is.
- **The selector relies on the dark panel being `:last-child`.** The last panel is never pinned, so GSAP never wraps it in a `pin-spacer`, and it stays the last child of `<main>`.
- **Known Safari behaviour: the top status strip.** It starts cream, but after you've visited the dark panel it can turn dark and stay dark until you scroll back to the top. iOS Safari 26–27 colours both toolbars from the page on its own schedule; nothing on the page controls it. Accepted trade-off.
- **What was tried and rejected** (iOS 27 Simulator, September 2026): switching the body to ink only while the panel was on screen (top strip stuck dark after scrolling back up); switching only `html`'s background, or a fixed ink band at the bottom edge (toolbar stayed cream); a solid nav (Safari used the nav's colour for both bars and didn't follow changes); inverting the nav to ink at the dark panel (no effect on the top strip). Approach from [nasedk.in, "iOS 26 Safari toolbar colors"](https://nasedk.in/blog/ios26-safari-toolbar-colors/), which reaches the same conclusion: design around Safari's toolbar colouring rather than drive it.
- **A torn bottom edge won't fix the toolbar.** The strip under Safari's toolbar is Safari's own chrome, coloured from the page background, not page content.
